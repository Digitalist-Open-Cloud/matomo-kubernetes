...
![Version: 11.0.65](https://img.shields.io/badge/Version-11.0.65-informational?style=flat-square) ![AppVersion: 5.7.1](https://img.shields.io/badge/AppVersion-5.7.1-informational?style=flat-square)
...
| matomo.dashboard.httpRoute | object | `{"annotations":{},"parentRefs":[]}` | HTTPRoute overrides for the dashboard, falls back to matomo.httpRoute when unset. |
| matomo.dashboard.httpRoute.annotations | object | `{}` | Annotations for the dashboard HTTPRoute, overrides matomo.httpRoute.annotations. |
| matomo.dashboard.httpRoute.parentRefs | list | `[]` | parentRefs for the dashboard HTTPRoute, overrides matomo.httpRoute.parentRefs. |
...
| matomo.httpRoute | object | `{"annotations":{},"enabled":false,"extralabels":{},"parentRefs":[]}` | HTTPRoute (Gateway API) settings, rendered alongside the Ingress when enabled. |
| matomo.httpRoute.annotations | object | `{}` | Default annotations for the HTTPRoute resources, can be overridden per surface. |
| matomo.httpRoute.enabled | bool | `false` | Enable HTTPRoute (gateway.networking.k8s.io/v1) resources. Opt-in; TLS is terminated at the Gateway listener. |
| matomo.httpRoute.extralabels | object | `{}` | Extra labels to add to the HTTPRoute resources. |
| matomo.httpRoute.parentRefs | list | `[]` | Default parentRefs (Gateways) to attach the HTTPRoutes to. |
...
| matomo.tracker.httpRoute | object | `{"annotations":{},"parentRefs":[]}` | HTTPRoute overrides for the tracker, falls back to matomo.httpRoute when unset. |
| matomo.tracker.httpRoute.annotations | object | `{}` | Annotations for the tracker HTTPRoute, overrides matomo.httpRoute.annotations. |
| matomo.tracker.httpRoute.parentRefs | list | `[]` | parentRefs for the tracker HTTPRoute, overrides matomo.httpRoute.parentRefs. |

<?php
  ...
  /**
   * @file
   * Matomo Configuration Tool
   *
   * Based on piwik-cli-setup by Ben Evans https://github.com/nebev/piwik-cli-setup/
   * which is under MIT License when this was created
   *
   * Usage:
   * php install.php /path/to/configuration.json
   */

  $config = json_decode(
    file_get_contents($argv[1]), TRUE
  );

  if (!is_array($config)) {
    die (
      "[ERROR] Cannot parse JSON file at [" . dirname(__FILE__)
      . "/install.example.json" . "]\n"
    );
  }

  /**
   * We get secrets as ENV variables. Let's add these to the configuration array
   */
  $config['password'] = getenv('MATOMO_ADMIN_PASSWORD');
  $config['database']['password'] = getenv('MATOMO_DB_PASSWORD');
  ...
  /**
   * TODO: Should perhaps be in a try-catch
   */
  $installer = new MatomoCliConfigure($config);
  $installer->configure();

  class MatomoCliConfigure {
    ...
    protected function initDBConnection() {
      ...
      if (array_key_exists('session_save_handler', $this->config)) {
        $config->General['session_save_handler']
          = $this->config['session_save_handler'];
      }
      ...
      Access::doAsSuperUser(
        function () use ($config_arr) {
          ...
          if (!$api->userExists($config_arr['login'])
            ...
            $api->addUser(
              $config_arr['login'], $config_arr['password'],
              $config_arr['email']
            );
    ...
    protected function addWebsite() {
      $nr_of_websites = (int)count($this->getWebsitesInstalled());
      $this->log( 'Got ' . $nr_of_websites . ' websites');
      if ($nr_of_websites === 0) {
        ...
        $result = Access::doAsSuperUser(
          function () use ($config_arr) {
            return APISitesManager::getInstance()->addSite(
              $config_arr['site_name'], $config_arr['site_url'], 0
            );
          }
        );
        $trustedHosts = [
    ...
    /**
     * Returns the list of all websites
     *
     * @return array for each site, an array of information (idsite, name, main_url, etc.)
     */
    protected function getWebsitesInstalled() {
      $config_arr = $this->config;
      $result = Access::doAsSuperUser(function () use ($config_arr) {
        return APISitesManager::getInstance()->getSitesWithAtLeastViewAccess();
      });
      return $result;
    }

    /**
     * Finishes the fake installation. Removes 'installation_in_progress' in
    ...
    protected function setGeo() {
      ...
      Option::set(
        'usercountry.location_provider', $this->config['geo_provider']
      );
     ...
     * [privacy] can exist in config. Sub settings are [anonymize_ip] and
     * [honor_do_not_track]
     */
     ...
     * [config] can exist. Each Key represents a section in the config file (
     * value should also be array) Each sub-array item in key->value represents
     * the config key and associated setting
     */
    protected function setConfigExtras() {
      $this->log('Setting up extra configuration');
      if (array_key_exists('extras', $this->config)
        && is_array(
          $this->config['extras']
        )
      ) {
    ...
    /**
     * Sets the branding
     * [branding] can exist in config, and can have keys [header_url]
     * representing image for header
     */
    protected function setBranding() {
      $this->log('Setting Branding');
      if (array_key_exists('branding', $this->config)
        && is_array(
          $this->config['branding']
        )
      ) {
        if (array_key_exists('header_url', $this->config['branding'])) {
          $image_contents = $this->getURL(
            $this->config['branding']['header_url']
          );
          file_put_contents(
            PIWIK_DOCUMENT_ROOT . '/misc/user/logo-header.png', $image_contents
          );
    ...
    protected function deactivatePlugins() {
      ...
      if (array_key_exists('deactivate_plugins', $this->config) && is_array($this->config['deactivate_plugins'])) {
        foreach ($this->config['deactivate_plugins'] as $plugin_to_deactivate) {
          echo exec(
              "php " . PIWIK_DOCUMENT_ROOT . "/console plugin:deactivate "
              . $plugin_to_deactivate
            ) . "\n";
        }
      }
    }
     ...
     * Looks for a [plugin_settings] key in config. Config then in key-value
     * form nested under plugin_name
     */
    protected function setPluginSettings() {
      $this->log('Updating Plugin Settings');
      if (array_key_exists('plugin_settings', $this->config)) {
        if (class_exists(
          "\\Piwik\\Plugins\\CorePluginsAdmin\\SettingsMetadata"
        )
        ) {
          foreach (
            $this->config['plugin_settings'] as $plugin_name => $plugin_settings
          ) {
            $this->log("Adding plugin settings for $plugin_name");
            $settings = new \Piwik\Settings\Storage\Backend\PluginSettingsTable(
              $plugin_name, ""
            );
          ...
          $this->log(
            "Cannot update plugin settings - Are you running Piwik 3?"
          );
        }
      }
    }
